...
class LoginScreen(QWidget):
    ...
    def __init__(self, app_context, parent: QWidget | None = None) -> None:
        ...
        intro = QFrame()
        intro.setStyleSheet("background: #143642; border-radius: 24px;")
        intro_layout = QVBoxLayout(intro)
        intro_layout.setContentsMargins(28, 28, 28, 28)
        title = QLabel("Adaptive English Exam")
        title.setObjectName("appTitle")
        subtitle = QLabel("Admin console for exam operations, imports, students, and question banks.")
    ...
    def _attempt_login(self) -> None:
        ...
        try:
            result = self._app_context.auth_controller.handle_admin_login(
                LoginRequest(username=self.username_edit.text(), password=self.password_edit.text())
            )
